...
public class Handler {
    ...
    private final IGetLoanApplicationUseCase getLoanApplicationUseCase;
    ...
    public Mono<ServerResponse> listenGetApplicationLoan(ServerRequest request) {
        ...
        return getLoanApplicationUseCase.getLoanApplication(status, page, size, token)
                .flatMap(response -> {
    ...
    @Operation(operationId = "calculateCapacity",
            summary = "calculate capacity a loan application",
            description = "The system receives the request information and sends it for processing.",
            requestBody = @RequestBody(required = true,
                    content = @Content(mediaType = "application/json",
                            schema = @Schema(implementation = DebtCapacityRequest.class)
                    )
            ),
            responses = {@ApiResponse(responseCode = "200",
                    description = "The capacity calculation for the request is in progress",
                    content = @Content(mediaType = "application/json",
                            schema = @Schema(implementation = GenericResponse.class)
                    )
            ), @ApiResponse(responseCode = "400",
                    ...
                    content = @Content(mediaType = "application/json",
                            schema = @Schema(implementation = ErrorResponseHandler.class),
                            examples = {
                                    @ExampleObject(name = "ApplicationStateConflict", value = "{\"timestamp\": \"2025-08-26T14:22:56.402Z\",\"status\":409,\"error\":\"BusinessException\",\"message\":\"The application status is different at pending\"}")
                            }
                    )
            ), @ApiResponse(responseCode = "500",
                    description = "Internal server error",
    ...
    @PreAuthorize("hasAuthority('ASESOR')")
    public Mono<ServerResponse> listenPostDebtCapacity(ServerRequest request) {

        String token = extractToken(request);

        return request.bodyToMono(DebtCapacityRequest.class)
                .flatMap(debtCapacityRequest -> RequestValidator.validate(debtCapacityRequest, validator))
                .map(loanApplicationMapper::toModel)
                .flatMap(application ->
                        loanApplicationUseCase.calculateCapacityApplication(application, token)
                                .flatMap(created -> {
                                    var response = new GenericResponse(LocalDateTime.now(), STATUS_OK, created);
                                    log.info(LOG_INFO_CALCULATE_CREATED);
                                    return ServerResponse.status(HttpStatus.OK).bodyValue(response);
                                })
                );
